        ...
        <el-table-column>
          <template v-slot="program">
            <div class="item clearFloat">
              ...
              <div class="itemText">
                <a :href="'/#/program?id='+program.row.program.id">
                  <h3>{{program.row.program.name}}</h3>
                </a>
                <a :href="'/#/djradio?id='+program.row.program.radio.id">
                  <span>{{program.row.program.radio.name}}</span>
                </a>
              </div>
...
<script>
import { getProToplist } from "../../../utils/api/djApi";
export default {
  data() {
    return {
      toplist: []
